...
export default class MessageChannel extends Component {
    constructor(){
        super();
        this.state= {
            data: [],
    ...
    async MessageElement(username, date, messageText, chatId, messageId){
        window.scrollTo(0,0);
        const sendMessage = () =>{
        ...
        if(this.state.isOpenMessage === true){
            this.setState({ messageElement: 
                <div className="OpenChat">
                    ...
                    <p>Время: { date }  </p>
                    <p>Список ответов {date.answer}</p>
                    <p>Список воментариев {date.сomments}</p>
                    <p>Ответ:</p>   
                        ...
                        if(data != null){
                            if(data.type === "channel"){
                                return <div className="ChatElement" key = {data.id} onClick = {async () => {
                                    ...
                                    this.MessageElement(data.username, convertDate(data.date), data.text, data.chatId, data.messageId); 
                                }}>
                                    <p>В: { data.nameFrom }</p>
