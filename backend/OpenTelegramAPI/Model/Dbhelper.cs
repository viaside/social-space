...
namespace web_app.Model
{
    public class DbHelper
    {
        ...
        public void AddMessage(MessageInfoModel messageInfoModel)
        {
            ...
            var result2 = _context.MessageInfo.Where(d => d.MessageId == messageInfoModel.MessageId).FirstOrDefault();
            ...
            var result = _context.MessageInfo
            .AsEnumerable()
            .GroupBy(s => new { s.MessageId, s.Answers, s.Comments })
            .SelectMany(g => g.Skip(1))
            .ToList();
            _context.MessageInfo.RemoveRange(result);
